...
<head>
    ...
    <title>E-Gov Dashboard | @yield('title', 'Admin')</title>
    
    {{-- 1. Script Anti-Flicker (Wajib di Head) --}}
    <script>
        // Cek preferensi pengguna saat halaman dimuat agar tidak berkedip
...
<body class="bg-white dark:bg-neutral-primary-soft antialiased transition-colors duration-200">
            ...
            <ul class="space-y-2 font-medium">
                ...
                <li>
                    ...
                    class="flex items-center p-2 rounded-base group {{ request()->routeIs('dashboard') ? 'bg-neutral-secondary-medium text-heading' : 'text-body hover:bg-neutral-secondary-medium hover:text-heading' }}">
                        ...
                        {{-- Icon Dashboard --}}
                        <svg class="w-5 h-5 transition duration-75 {{ request()->routeIs('dashboard') ? 'text-heading' : 'text-neutral-tertiary-medium group-hover:text-heading' }}" fill="currentColor" viewBox="0 0 20 20">
                ...
                <li>
                    ...
                    class="flex items-center p-2 rounded-base group {{ request()->routeIs('permohonan') ? 'bg-neutral-secondary-medium text-heading' : 'text-body hover:bg-neutral-secondary-medium hover:text-heading' }}">
                        ...
                        {{-- Icon Permohonan --}}
                        <svg class="w-5 h-5 transition duration-75 {{ request()->routeIs('permohonan') ? 'text-heading' : 'text-neutral-tertiary-medium group-hover:text-heading' }}" fill="currentColor" viewBox="0 0 20 20">
    ...
    {{-- 2. SCRIPT LOGIC TOMBOL (WAJIB ADA DI SINI) --}}
    <script>
        var themeToggleDarkIcon = document.getElementById('theme-toggle-dark-icon');
        var themeToggleLightIcon = document.getElementById('theme-toggle-light-icon');
        var themeToggleBtn = document.getElementById('theme-toggle');

        // 1. Cek kondisi awal saat load: Tentukan icon mana yang muncul
        if (localStorage.getItem('color-theme') === 'dark' || (!('color-theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
            // Jika mode gelap, sembunyikan icon bulan (karena kita mau tombol untuk ke terang), munculkan matahari
            themeToggleDarkIcon.classList.remove('hidden'); 
            themeToggleLightIcon.classList.add('hidden');
        } else {
            // Jika mode terang, sembunyikan matahari, munculkan bulan
