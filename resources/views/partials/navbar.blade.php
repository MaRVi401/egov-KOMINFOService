<nav
    ...
    <div class="max-w-7xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{ asset('assets/images/landingPages/logo-diskominfo.png') }}" class="h-8 hidden md:block"
                alt="Logo" />
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-2 rtl:space-x-reverse items-center">
            @auth
                <a href="{{ route(auth()->user()->role . '.dashboard') }}"
                    class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 transition">Dashboard</a>
            @else
                <button type="button" onclick="window.location='{{ route('login') }}'"
                    ...
                    <svg class="w-5 h-5 me-2 -ms-1 text-gray-800 dark:text-white" aria-hidden="true"
