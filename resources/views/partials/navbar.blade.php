<nav class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800">
    <div class="max-w-7xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-2 rtl:space-x-reverse items-center">
            @auth
                @php
                    $roleRoute = str_replace('_', '-', auth()->user()->role) . '.dashboard';
                @endphp

                @if (Route::has($roleRoute))
                    <a href="{{ route($roleRoute) }}"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition">
                        Dashboard
                    </a>
                @else
                    <a href="/"
                        class="text-white bg-red-600 hover:bg-red-700 font-medium rounded-lg text-sm px-5 py-2.5 transition">
                        Dashboard (Route Missing)
                    </a>
                @endif
            @else
                <button type="button" onclick="window.location='{{ route('login') }}'"
                    class="inline-flex items-center text-gray-900 dark:text-white bg-transparent border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-700 font-medium rounded-lg text-sm px-4 py-2.5 transition duration-300">
                    <svg class="w-5 h-5 me-2 -ms-1 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 16">
