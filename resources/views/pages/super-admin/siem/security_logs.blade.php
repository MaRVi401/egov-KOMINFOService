...
@section('content')
    <div class="p-4 mt-14">
        {{-- Breadcrumb sama seperti index di atas, tambahkan sub-menu "Security Logs" --}}
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Pencatatan aktivitas login, logout, dan indikasi ancaman siber.</p>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($logs as $log)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                    ...
                                    @if($log->user)
                                        <span class="block text-xs text-blue-500">Terdaftar sebagai: {{ $log->user->nama }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4"><span class="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{{ $log->ip_address }}</span></td>
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="text-gray-600 dark:text-gray-300">{{ ucfirst($log->tipe_event) }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($log->is_suspicious)
                                        <span class="bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded border border-red-400 animate-pulse">SUSPICIOUS (Alerted)</span>
                                    @else
                        ...
                        @empty
                            <tr><td colspan="5" class="px-6 py-4 text-center">Tidak ada log keamanan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
